...
class App_UploadHandler {

    private $options;

    function __construct($options=null) {
        $sessionCustom = new Zend_Session_Namespace('custom');
        $gallery = $sessionCustom->galleryObject;
//        Zend_Debug::fdump($gallery,'$gallery');

        $this->options = array(
            'gallery_id' => $sessionCustom->galleryObject->id,
            'script_url' => $_SERVER['PHP_SELF'],
            'upload_dir' => $_SERVER['DOCUMENT_ROOT'] . $gallery->path,
            'upload_url' => $gallery->path,
            'delete_url' => '/upload/index/process/',
            'param_name' => 'files',
            ...
            'image_versions' => array(
                 ...
                 */
                'thumbnail' => array(
                    'upload_dir' => $_SERVER['DOCUMENT_ROOT'] . '/thumbnails/',
...
//        Zend_Debug::fdump($file, '$file');
//        Zend_Debug::fdump($this->options, '$this->options');

        $fileModels = new Model_Files();
        $fileModels->saveFile($this->options['gallery_id'], $file->name, $file_path, $file->url, $file->size, $file->type);
        return $file;
    }
...
//        Zend_Debug::fdump($upload,'$upload');
        $info = array();
...
//        Zend_Debug::fdump($info, '$info');
    ...
    public function delete() {
        ...
        $fileModels = new Model_Files();
        $fileFoDeleting = $fileModels->findByUrl($this->options['upload_url'] .urlencode($file_name));
//        Zend_Debug::fdump($this->options['upload_url'] . $file_name,'$this->options[upload_url] . $file_name');
//        Zend_Debug::fdump($fileFoDeleting,'$fileFoDeleting');
        $fileFoDeleting->delete();
        header('Content-type: application/json');
